...
For network findings, include the current Codex version, proxy-alignment classification, redacted endpoint sources and ports, exact listener status, and whether port drift is suspected. Distinguish native proxy handling from an explicit environment-variable fallback. Do not infer that a browser working proves Codex uses the same endpoint. If `.codex/.env` exists, report presence only and do not claim the current client automatically loads it. For session findings, distinguish transcript-file presence from CLI visibility, Desktop sidebar visibility, and resumability; do not infer one from another.
